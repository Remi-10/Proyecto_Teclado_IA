...
class KeyboardLogic:
    def __init__(self):
        ...
        self.max_length = 15  # longitud máxima de palabra
        self.model_path = "word_completion_lstm.pth"
        self._prepare_data()
        self._train_or_load_model()

        # Layout del teclado (puedes ajustar según tu UI)
        self.key_layout = [
            ['1', '2', '3', '4', '5', '6', '7', '8', '9', '0'],
            ['q', 'w', 'e', 'r', 't', 'y', 'u', 'i', 'o', 'p'],
            ['a', 's', 'd', 'f', 'g', 'h', 'j', 'k', 'l', 'ñ'],
    ...
    def _prepare_data(self):
        ...
        self.char_to_idx = {c: i+1 for i, c in enumerate(chars)}  # 0 para padding
    ...
    def _predict_words(self, prefix, n=3):
        # Si el prefijo contiene caracteres desconocidos, no sugerir nada
        ...
        with torch.no_grad():
            ...
            # Buscar las N palabras más cercanas en el entrenamiento que empiecen igual
            suggestions = []
            for word in self.words:
                ...
                if len(suggestions) >= n:
                    break
            # Si la predicción del modelo es válida y no está en las sugerencias, agregarla
            if pred_word.startswith(prefix) and pred_word not in suggestions:
                suggestions.insert(0, pred_word)
